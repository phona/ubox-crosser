...
### Requirement: Healthz endpoint returns OK on GET

The proxy server SHALL expose an HTTP endpoint at the path `/healthz` that returns HTTP status 200 with a JSON body `{"status":"ok"}` and `Content-Type: application/json` header.

#### Scenario: Successful health check

- **WHEN** a client sends `GET /healthz` to the health HTTP listener
- **THEN** the server responds with HTTP 200, header `Content-Type: application/json`, and body `{"status":"ok"}`

### Requirement: Healthz endpoint rejects non-GET methods

The healthz endpoint SHALL return HTTP 405 Method Not Allowed for any HTTP method other than GET on the `/healthz` path. The response SHALL include an `Allow: GET` header.

#### Scenario: POST request to healthz endpoint

- **WHEN** a client sends `POST /healthz` to the health HTTP listener
- **THEN** the server responds with HTTP 405 and an `Allow: GET` header

#### Scenario: PUT request to healthz endpoint

- **WHEN** a client sends `PUT /healthz` to the health HTTP listener
- **THEN** the server responds with HTTP 405 and an `Allow: GET` header

### Requirement: Unknown paths return 404

The health HTTP listener SHALL return HTTP 404 Not Found for any request path other than `/healthz`.
