...
// The spec requires a 7-character short SHA injected as -X main.GitSHA=<sha>.
...
//
// These two scenarios share one test parameterised via EXPECTED_BUILD_ID:
//   - Set EXPECTED_BUILD_ID=ci-42 (matching BUILD_ID=ci-42 on the server) to exercise S3.
//   - Leave EXPECTED_BUILD_ID unset to exercise S4 (expects the default value "dev").
...
// Sends no Authorization header; expects 200 OK.
func TestBuildinfoEndpoint_S7_NoAuthRequired(t *testing.T) {
	...
	// No Authorization header is set — endpoint must respond 200 regardless.
