...
// protocolTestService is a separate service name used by protocol-level tests
// to avoid overriding the real client's controller for "test_service".
const protocolTestService = "protocol_test"
...
// This test MUST run first (Go runs tests in source order) to avoid controller conflicts.
func TestAuthServerTunnel(t *testing.T) {
	...
	// We need to speak SOCKS5 protocol to reach the echo-server.
...
// Uses protocolTestService to avoid overriding the real client's controller.
